...
public class DocumentSearchService implements IDocumentSearchService {
    ...
    public Result<PageResultVO<GlobalSearchResultVO>> search(GlobalSearchDTO dto, PageRequestDTO page) {
        ...
        Query query = MultiMatchQuery.of(q -> q.query(keyWord)
                .fields("title", "content"))._toQuery();
        BoolQuery boolQuery = BoolQuery.of(b->b.must( MultiMatchQuery.of(q->q.query(filter).fields("filter"))._toQuery()));
        Query queryBool = boolQuery._toQuery();
    ...
    private Result<PageResultVO<GlobalSearchResultVO>> manualPagination(List<GlobalSearchResultVO> voList, PageRequestDTO page) {
        ...
        // 构建分页结果VO
        PageResultVO<GlobalSearchResultVO> pageResult = new PageResultVO<>();
        pageResult.setList(pagedList);
        pageResult.setTotal((long) totalItems);
